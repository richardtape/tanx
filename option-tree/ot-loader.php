    ...
    private function load_languages() {
      ...
      if ( OT_PLUGIN_MODE ) {
        ...
        if ( apply_filters( 'ot_child_theme_mode', false ) == true ) {
          ...
          $start = explode( get_stylesheet(), str_replace( '\\', '/', dirname( __FILE__ ) ) );
          $path = ltrim( end( $start ), '/' );
          define( 'OT_LANG_DIR', trailingslashit( trailingslashit( get_stylesheet_directory() ) . $path ) . trailingslashit( 'languages' ) . 'theme-mode' );

        } else {

          $start = explode( get_template(), str_replace( '\\', '/', dirname( __FILE__ ) ) );
          $path = ltrim( end( $start ), '/' );
    ...
    public function global_admin_css() {
      ...
      <style>
        ...
        #option-tree-header #option-tree-logo a:before {
          font: normal ' . $fontsize . '/1 "icomoon" !important;
          speak: none;
          padding: 6px 0;
          height: 34px;
          width: 20px;
